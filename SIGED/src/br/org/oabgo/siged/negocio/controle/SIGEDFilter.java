...
public class SIGEDFilter implements Filter, SIGEDConstantes {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		try {
			...
			if (this.hSession != null && this.hRequest.isRequestedSessionIdValid()) {
				usuarioLogado = (UsuarioTO) this.hSession.getAttribute(SIGEDConstantes.USUARIO_LOGADO);
			}
			...
			if (usuarioLogado != null) {
				...
				if (this.path.equals(URL_CADASTRAR_SENHA)) {
					request.getRequestDispatcher(URL_CADASTRAR_SENHA).forward(request, response);
				} else {
